...
(defvar-local verb--response-original-buffer nil
  "Original buffer where the request came from.
It could be either the Org buffer, in which case this is the same
as `current-buffer', or a buffer genereated by
`verb--generate-response-buffer'.")
(put 'verb--response-original-buffer 'permanent-local t)
...
(defun verb--apply-map-response ()
  ...
  (when-let ((res verb-last)
    ...
    (with-current-buffer verb--response-original-buffer
      (funcall fn res))))
...
(defun verb--request-spec-callback (status rs response-buf original-buf start
                                    timeout-timer where num)
...
ORIGINAL-BUF points to the buffer where the request came from.
Could be either the current buffer or a buffer generated by
`verb--generate-response-buffer'. This value will be stored in
the local variable `verb--response-original-buffer' in
RESPONSE-BUF.
...
NUM is this request's identification number."
  ...
  (let ((elapsed (- (time-to-seconds) start))
    ...
    (with-current-buffer response-buf
      ...
      ;; Also set the private variable with the original buffer
      (setq verb--response-original-buffer original-buf)
