...
Then, move the point to one of the level 2 headers marked with `**`), and press <kbd>C-c C-c C-r</kbd> to send an HTTP request.
...
If you wish to see the HTTP response headers, use the `verb-toggle-show-headers` command, from the response body buffer. By default, it is bound to <kbd>C-c C-c C-r</kbd>.
...
To close the response headers buffer, use the `verb-toggle-show-headers` command again (<kbd>C-c C-c C-r</kbd>) while the response body buffer is selected.
...
If you wish to check what request is described by a certain heading, taking into considaration all its parent headings, use the `verb-preview-request-on-point` command. By default, it is bound to <kbd>C-c C-r C-f</kbd>. When used, to HTTP request will be sent, but instead some of its attributes will be shown on the minibuffer using `message`.
