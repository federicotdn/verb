...
- `eww`: Perform the request specification using EWW (Emacs Web Wowser). This will only work on `GET` requests.

Each export function is bound to a key by default. The keys are:
- `curl`: <kbd>C-c C-r C-u</kbd>.
- `verb`: <kbd>C-c C-r C-b</kbd>.
- `eww`: <kbd>C-c C-r C-w</kbd>.
